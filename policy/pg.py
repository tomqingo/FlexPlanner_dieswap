...
class PGPolicy(BasePolicy):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # else:
        #     dist = self.dist_fn(logits)
        assert isinstance(logits, Batch), "logits should be a Batch"
        

        if self._deterministic_eval and not self.training:
            raise NotImplementedError("deterministic evaluation is not implemented yet")
            ...
            dist = Batch()
            act = Batch()
            for dist_name in logits.keys():
                # print("Before: ", logits[dist_name])
                dist_curr = self.dist_fn[dist_name](logits[dist_name])
                # print("After: ", dist_curr)
                act_curr = dist_curr.sample()
                # print("current action: ", act_curr)
                dist[dist_name] = dist_curr
                act[dist_name] = act_curr
