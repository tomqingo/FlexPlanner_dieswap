...
class BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def __init__(
        ...
        use_last_step_reward_to_replace_other_steps: bool = False,
        add_last_step_reward_to_other_steps: bool = False,
    ) -> None:
        super().__init__()
        ...
        self._compile()
        self.use_last_step_reward_to_replace_other_steps = use_last_step_reward_to_replace_other_steps
        self.add_last_step_reward_to_other_steps = add_last_step_reward_to_other_steps
        assert not (self.use_last_step_reward_to_replace_other_steps and self.add_last_step_reward_to_other_steps), "use_last_step_reward_to_replace_other_steps and add_last_step_reward_to_other_steps can't be True at the same time"
    ...
    def map_action(self, act: Union[Batch, np.ndarray]) -> Union[Batch, np.ndarray]:
        ...
        if isinstance(self.action_space, gym.spaces.Box) and \
            ...
            #print("bound: ", self.action_bound_method)
            if self.action_bound_method == "clip":
                act = np.clip(act, -1.0, 1.0)
            elif self.action_bound_method == "tanh":
                act = np.tanh(act)

            #print("scale: ", self.action_scaling)
    ...
    def use_last_step_reward_to_replace_other_steps_fn(self, buffer: ReplayBuffer) -> ReplayBuffer:
        n_steps = buffer.rew.shape[0]
        for i in reversed(range(n_steps-1)): # avoid out of index
            if not buffer.terminated[i]:
                buffer.rew[i] = buffer.rew[i+1]
        return buffer
    

    def add_last_step_reward_to_other_steps_fn(self, buffer: ReplayBuffer) -> ReplayBuffer:
        n_steps = buffer.rew.shape[0]
        last_step_rew = 0
        for i in reversed(range(n_steps)): # from last step to first step
            if buffer.terminated[i]:
                last_step_rew = buffer.rew[i]
            else:
                buffer.rew[i] += last_step_rew
        return buffer
    ...
    def update(self, sample_size: int, buffer: Optional[ReplayBuffer],
        ...
        assert not (self.use_last_step_reward_to_replace_other_steps and self.add_last_step_reward_to_other_steps), "use_last_step_reward_to_replace_other_steps and add_last_step_reward_to_other_steps can't be True at the same time"
        if self.use_last_step_reward_to_replace_other_steps:
            buffer = self.use_last_step_reward_to_replace_other_steps_fn(buffer)

        if self.add_last_step_reward_to_other_steps:
            buffer = self.add_last_step_reward_to_other_steps_fn(buffer)
