...
class FPInfo:
    ...
    def calc_via(self) -> int:
        """calculate the number of vias"""
        num_via = 0
        #print("net_len: ", len(self.net_info))
        # optimize the number of the vias
        for net in self.net_info:
            num_via += net.is_cut()
        return num_via
    ...
    def calc_area_ratio(self) -> float:
        ...
        if len(area_layer)==1 or area_layer[1] == 0:
            area_ratio = 10000000
        else:
    ...
    def calc_num_ratio(self) -> float:
        ...
        if len(num_block_layer)==1 or num_block_layer[1] == 0:
