...
fp_info, df_partner = circuit_dataloader.construct_fp_info_func(args.circuit, args.area_util, num_grid_x, num_grid_y, 
                                                    args.num_alignment, args.alignment_rate, args.alignment_sort, args.num_preplaced_module, args.add_virtual_block, args.num_layer, False, False, args.add_halo, args.halo_width, args.halo_height, args.ratio_range)
...
# print("shared_encoder_input_channel:", shared_encoder_input_channel)

shared_encoder_cls = getattr(model, args.shared_encoder_cls)

# print("graph: ", args.graph)
# print("final_shape: ", args.shared_encoder_final_shape)
...
layer_decider = LayerDeciderClass(fp_info.num_layer, layer_decider_input_dim, layer_decider_hidden_dim, 
                                args.async_place_share_with_critics, args.die_embedding, args.async_place_input_sequence, args.input_layer_sequence, args.async_place)
# else:
#     layer_decider = None
...
actor = model.Actor(
    ...
    input_partner_die=args.input_partner_die, input_alignment_mask=args.input_alignment_mask, async_place=args.async_place, input_next_block=args.input_next_block, use_ready_layers_mask=False, 
...
dist_cls = {
    ...
    "layerdst": torch.distributions.Categorical
}
# else:
    # dist_cls = {
    #     "pos": torch.distributions.Categorical,
...
clip_loss_coef = ClipLossCoef(args.pos_coef, args.ratio_coef, args.async_place_coef, args.async_place) # args for clip loss
entropy_loss_coef = EntropyLossCoef(async_place_coef=args.async_place_entropy_coef, async_place=args.async_place) # args for entropy loss
...
# Why should we set num_env 8?
buffer_size = num_env * episode_per_collect_per_env * episode_len

# print("calculation results: ", num_env, episode_per_collect_per_env, episode_len, buffer_size)
...
if not wiremask_bbo and args.train:
    if args.statistics is None:
        ...
        print("statistics buffer size: ", num_env_test, episode_per_collect_per_env, episode_len)
        # print("env length: ", len(collect_envs))
...
execution_time = train_time_end - train_time_start
