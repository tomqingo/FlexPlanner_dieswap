...
class BaseTrainer(ABC):
    ...
    def __init__(
        ...
        # print("test_fn: ", self.test_fn)
        # print("stop_fn: ", self.stop_fn)
    ...
    def reset(self) -> None:
        ...
        if self.test_collector is not None:
            ...
            self.best_via, self.best_via_std = \
                test_result["via"], test_result["via_std"]
    ...
    def test_step(self) -> Tuple[Dict[str, Any], bool]:
        ...
        via, via_std = test_result["via"], test_result["via_std"]
        ...
        if self.best_via > via:
            self.best_via = via
            self.best_via_std = via_std
        ...
        if not self.is_run:
            ...
            test_stat = {
                ...
                "test_via": via,
                "test_via_std": via_std,

                "best_via": self.best_via,
                "best_via_std": self.best_via_std,
